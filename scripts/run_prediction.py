...
def find_sportstake_columns(df, main_count, filename):
    # Strategy 1: look for gameXscore or gameXresults (case insensitive)
    ...
    if matches:
        ...
        if len(result) == main_count:
            return result
    # Strategy 2: any column that contains the word 'result' or 'score' and a digit
    alt_matches = []
    for col in df.columns:
    ...
    # Strategy 3: take first main_count columns that are not obvious metadata
    ...
    for csv_name, cfg in LOTTERY_RULES.items():
        ...
        if lottery_type == "balls":
            ...
            try:
                ...
                print(f"Error processing {csv_name}: {e}")
                # Fallback: print columns to help debug
                print("Available columns:", df.columns.tolist())
                continue
            # Convert each column to outcomes
            ...
            df = df[df['main_numbers'].apply(len) == main_count]
            if len(df) == 0:
                print(f"WARNING: No valid rows for {csv_name}. Check if columns contain scores/outcomes.")
                # Optional: print first few rows to see data
                # print(df[result_cols].head())
                continue

        if len(df) == 0:
            print(f"No valid rows found in {csv_name}")
            continue
