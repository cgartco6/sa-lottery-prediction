...
def parse_score_to_outcome(score_str):
    """Convert a score string like '3-1' to outcome: 1=home, 2=draw, 3=away."""
    ...
    if '-' in s:
        ...
        if len(parts) == 2:
            try:
                ...
                pass
    # If it's already a single digit 1,2,3
...
def find_sportstake_columns(df, main_count, filename):
    """Find columns that contain match results (score or result code)."""
    # Look for gameXresults or gameXscore columns
    candidates = []
    for i in range(1, main_count + 1):
        for colname in [f"game{i}results", f"game{i}score", f"game{i}_results", f"game{i}_score"]:
            if colname in df.columns:
                candidates.append((i, colname))
                break
    if len(candidates) == main_count:
        # Sort by the match number
        candidates.sort(key=lambda x: x[0])
        return [col for _, col in candidates]
    # Fallback: any column containing 'result' or 'score' with a digit
    all_candidates = []
    for col in df.columns:
        match = re.search(r'game(\d+)(?:results|score)', col, re.IGNORECASE)
        if match:
            num = int(match.group(1))
            all_candidates.append((num, col))
    all_candidates.sort()
    if len(all_candidates) >= main_count:
        return [col for _, col in all_candidates[:main_count]]
    raise ValueError(f"Cannot find {main_count} Sportstake result columns in {filename}")

def find_number_columns(df, main_count, filename):
    """Find ball columns: expects 'ball1', 'ball2', ..."""
    candidates = [f"ball{i}" for i in range(1, main_count+1) if f"ball{i}" in df.columns]
    if len(candidates) == main_count:
        return candidates
    # Fallback: any 'ball' followed by digits
    all_ball = []
    for col in df.columns:
...
def main():
    ...
    for csv_name, cfg in LOTTERY_RULES.items():
        ...
        # ========== EXTRACT MAIN NUMBERS (BALLS OR SPORTSTAKE OUTCOMES) ==========
        if lottery_type == "balls":
            ...
            try:
                ...
                print(f"Error processing {csv_name}: {e}")
                continue
            # Convert each score/result column to outcome (1,2,3)
        ...
        # ========== BONUS COLUMN (only for ball lotteries with bonus) ==========
        ...
        # ========== GROUP ANALYSIS (works for both types) ==========
