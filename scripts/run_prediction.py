...
# ------------------------------------------------------------
LOTTERY_RULES = {
    "DAILYLOTTO-RESULTS":          {"main_count": 5, "max_main": 36, "has_bonus": False, "type": "balls"},
    "DAILYLOTTO-PLUS-RESULTS":     {"main_count": 5, "max_main": 36, "has_bonus": False, "type": "balls"},
    "LOTTO-RESULTS":               {"main_count": 6, "max_main": 52, "has_bonus": True, "bonus_max": 20, "type": "balls"},
    "LOTTO-PLUS1-RESULTS":         {"main_count": 6, "max_main": 52, "has_bonus": True, "bonus_max": 20, "type": "balls"},
    "LOTTO-PLUS2-RESULTS":         {"main_count": 6, "max_main": 52, "has_bonus": True, "bonus_max": 20, "type": "balls"},
    "POWERBALL-RESULTS":           {"main_count": 5, "max_main": 50, "has_bonus": True, "bonus_max": 20, "type": "balls"},
    "POWERBALL-PLUS-RESULTS":      {"main_count": 5, "max_main": 50, "has_bonus": True, "bonus_max": 20, "type": "balls"},
    "SPORTSTAKE-1X2-RESULTS":      {"main_count": 6, "max_main":  4, "has_bonus": False, "type": "sportstake"},
    "SPORTSTAKE-SS08-RESULTS":     {"main_count": 8, "max_main":  4, "has_bonus": False, "type": "sportstake"},
}

# ------------------------------------------------------------
# HELPER FUNCTIONS
# ------------------------------------------------------------
def parse_score_to_outcome(val):
    if pd.isna(val):
        return None
    s = str(val).strip()
    ...
    if '-' in s:
        ...
        if len(parts) == 2:
            try:
                ...
                pass
    if s.upper() == 'H': return 1
    if s.upper() == 'D': return 2
    if s.upper() == 'A': return 3
    return None

def find_number_columns(df, main_count, filename):
    candidates = [f"ball{i}" for i in range(1, main_count+1) if f"ball{i}" in df.columns]
    if len(candidates) == main_count:
        return candidates
    all_ball = []
    for col in df.columns:
        m = re.search(r'ball(\d+)', col, re.IGNORECASE)
        if m:
            all_ball.append((int(m.group(1)), col))
    all_ball.sort()
    if len(all_ball) >= main_count:
        return [col for _, col in all_ball[:main_count]]
    raise ValueError(f"Cannot find {main_count} ball columns")

def find_bonus_column(df, filename):
    for col in df.columns:
        if 'bonus' in col.lower() and 'ball' in col.lower():
            return col
        if 'bonus' in col.lower() and not any(x in col.lower() for x in ['payout','div']):
            return col
    return None

def find_sportstake_columns(df, main_count, filename):
    pattern = re.compile(r'game(\d+)(?:score|results|_score|_results)', re.IGNORECASE)
    matches = []
    for col in df.columns:
        m = pattern.search(col)
        if m:
            num = int(m.group(1))
            matches.append((num, col))
    if matches:
        matches.sort()
        result = [col for _, col in matches[:main_count]]
        if len(result) == main_count:
            return result
    alt_matches = []
    for col in df.columns:
        if 'result' in col.lower() or 'score' in col.lower():
            nums = re.findall(r'\d+', col)
            if nums:
                alt_matches.append((int(nums[0]), col))
    alt_matches.sort()
    if len(alt_matches) >= main_count:
        return [col for _, col in alt_matches[:main_count]]
    exclude = re.compile(r'date|time|home|away|team|payout|div|jackpot|draw', re.IGNORECASE)
    candidates = [col for col in df.columns if not exclude.search(col)]
    if len(candidates) >= main_count:
        return candidates[:main_count]
    raise ValueError(f"Cannot find {main_count} Sportstake columns")

# ------------------------------------------------------------
# CLASSES (DataProcessor, GroupAnalyzer, NumberPredictor, TicketOptimizer, TicketChecker)
# ------------------------------------------------------------
class DataProcessor:
    @staticmethod
...
# ------------------------------------------------------------
# MAIN PROCESSING FUNCTION (returns tickets and info for a single lottery)
# ------------------------------------------------------------
def process_lottery(csv_name, cfg, df, data_dir):
    """Process a single lottery DataFrame and return generated tickets."""
    has_bonus = cfg['has_bonus']
    max_main = cfg['max_main']
    main_count = cfg['main_count']
    lottery_type = cfg['type']
    
    # Extract main numbers
    if lottery_type == "balls":
        ...
        result_cols = find_sportstake_columns(df, main_count, csv_name)
        for col in result_cols:
            df[col] = df[col].apply(parse_score_to_outcome)
        df['main_numbers'] = df[result_cols].values.tolist()
        df['main_numbers'] = df['main_numbers'].apply(lambda row: [int(x) for x in row if x is not None])
        df = df[df['main_numbers'].apply(len) == main_count]
    ...
    # Bonus
    if has_bonus and lottery_type == "balls":
    ...
    # Generate tickets
    tickets_6 = optimizer.generate_tickets(6, main_count, has_bonus)
    tickets_13 = optimizer.generate_tickets(13, main_count, has_bonus)
    ...
    return {
        ...
        'df': df  # needed for win checking later
    }, None
...
def main():
    ...
    parser.add_argument('--lottery', default='all')
    ...
    # First pass: print full analysis for all lotteries (original behaviour)
    for csv_name, cfg in LOTTERY_RULES.items():
        ...
        result, err = process_lottery(csv_name, cfg, df, data_dir)
        if err:
            print(f"Error processing {csv_name}: {err}")
            continue
        
        print("="*70)
        print(f"  {csv_name}  |  Predicted group pattern: {result['pred_groups']}")
        print("="*70)
        focus = [int(x) for x in result['pred_numbers']['main']]
        print(f"  🎯 Suggested focus numbers: {focus}", end='')
        if result['has_bonus'] and result['pred_numbers'].get('bonus'):
            print(f"  + Bonus: {result['pred_numbers']['bonus']}")
        else:
            print()
        
        for (num_tickets, tickets) in [(6, result['tickets_6']), (13, result['tickets_13'])]:
            print(f"\n  ┌── {num_tickets} TICKET(S) ──")
            for i, t in enumerate(tickets):
                main_str = " ".join(f"{n:2d}" for n in t['main'])
                if result['has_bonus'] and t.get('bonus'):
            ...
            latest = result['df'].iloc[-1].to_dict()
            winners = []
            ...
            if winners:
                date_col = next((c for c in result['df'].columns if 'date' in c.lower()), None)
    ...
    # ------------------------------------------------------------------
    # NEW: Ask user which lottery to play today
    # ------------------------------------------------------------------
    print("\n" + "="*70)
    print("  PLAY TODAY")
    print("="*70)
    lotteries_available = [name for name in LOTTERY_RULES.keys() if (data_dir / f"{name}.csv").exists()]
    print("Available lotteries:")
    for i, name in enumerate(lotteries_available, 1):
        print(f"  {i}. {name}")
    print("\nEnter the numbers of the lotteries you want to play today (comma-separated).")
    print("Example: 1,3,5   or   'all'")
    choice = input("Your choice: ").strip().lower()
    
    selected = []
    if choice == 'all':
        selected = lotteries_available
    else:
        parts = choice.replace(' ', '').split(',')
        for p in parts:
            if p.isdigit():
                idx = int(p) - 1
                if 0 <= idx < len(lotteries_available):
                    selected.append(lotteries_available[idx])
                else:
                    print(f"Invalid number: {p}, skipping.")
            else:
                # try to match by name
                matched = [n for n in lotteries_available if n.lower() == p.lower()]
                if matched:
                    selected.append(matched[0])
                else:
                    print(f"Unknown lottery: {p}")
    
    if not selected:
        print("No valid selection. Exiting.")
        return
    
    # Ask number of tickets per played lottery
    try:
        play_tickets = int(input("How many tickets per played lottery? (default 6): ") or "6")
    except:
        play_tickets = 6
    
    save_to_file = input("Save play slip to file? (y/n, default n): ").strip().lower() == 'y'
    
    output_lines = []
    output_lines.append("="*70)
    output_lines.append(f"PLAY SLIP - {pd.Timestamp.now().strftime('%Y-%m-%d %H:%M')}")
    output_lines.append("="*70)
    
    for lot in selected:
        csv_path = data_dir / f"{lot}.csv"
        df = pd.read_csv(csv_path)
        result, err = process_lottery(lot, LOTTERY_RULES[lot], df, data_dir)
        if err:
            print(f"Could not process {lot}: {err}")
            continue
        
        # Generate play tickets (using the same hot numbers logic, but with user-specified count)
        # We need to regenerate tickets with the user's desired count. Since process_lottery only gives 6 and 13,
        # we'll re-create the optimizer from the result's df.
        df_proc = result['df']
        has_bonus = result['has_bonus']
        max_main = LOTTERY_RULES[lot]['max_main']
        main_count = result['main_count']
        # Recompute hot numbers from df_proc
        all_mains = [n for sub in df_proc['main_numbers'] for n in sub]
        top_mains = pd.Series(all_mains).value_counts().head(15).index.tolist()
        optimizer = TicketOptimizer(top_mains, result['pred_numbers'].get('bonus'), max_main, main_count)
        play_tickets_list = optimizer.generate_tickets(play_tickets, main_count, has_bonus)
        
        print(f"\n--- {lot} ({play_tickets} tickets) ---")
        output_lines.append(f"\n{lot} ({play_tickets} tickets)")
        for i, t in enumerate(play_tickets_list, 1):
            main_str = " ".join(f"{n:2d}" for n in t['main'])
            if has_bonus and t.get('bonus'):
                line = f"  Ticket {i:2d}: {main_str}   [B: {t['bonus']:2d}]"
            else:
                line = f"  Ticket {i:2d}: {main_str}"
            print(line)
            output_lines.append(line)
    
    if save_to_file:
        filename = f"play_slip_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}.txt"
        with open(filename, 'w') as f:
            f.write("\n".join(output_lines))
        print(f"\nPlay slip saved to {filename}")
